    ...
    def load(self, level: int) -> None:
        ...
        for line in fileinput.input(os.path.join("Assets", "Levels", "level" + str(level) + ".dat")):
            for currentBrick in line:
                if currentBrick == "1":
                    brick = Brick(positions = [x, y], sprite = pygame.image.load(GameConstants.SPRITE_BRICK), game = self.__game)
                    self.__bricks.append(brick)
                    self.__amountOfBricksLeft += 1
                elif currentBrick == "2":
                    brick = SpeedBrick(positions = [x, y], sprite = pygame.image.load(GameConstants.SPRITE_SPEED_BRICK), game = self.__game)
                    self.__bricks.append(brick)
                    self.__amountOfBricksLeft += 1
                elif currentBrick == "3":
                    brick = LifeBrick(positions = [x, y], sprite = pygame.image.load(GameConstants.SPRITE_LIFE_BRICK), game = self.__game)
                    self.__bricks.append(brick)
                    self.__amountOfBricksLeft += 1
